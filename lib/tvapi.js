...
function createTelevisionApi(options) {
    ...
    function rememberLink(sourceUrl, title) {
    ...
    function listLinks() {
        return Array.from(sentLinks.values()).sort((left, right) => right.lastSentAt - left.lastSentAt);
    }
    ...
    async function attachSubtitleCues(publication) {
        ...
        const isLocalFile = selected.localPath !== undefined && selected.localPath !== EMPTY_STRING;
        if (isLocalFile) {
            selected.cues = subtitles.readSidecarCues(selected.localPath);
        } else if (selected.cues === null || selected.cues === undefined) {
    ...
    function buildPlaybackPayload(publication) {
        ...
        return {
            ...
            maxVideoHeight: Number(publication.selectedRenditionId) || 0,
    ...
    async function publishAndPush(response, resolved, subtitleId, renditionId) {
        try {
            const isDirectStream = resolved.streamUrl !== undefined && resolved.streamUrl !== EMPTY_STRING;
            if (renditionId !== EMPTY_STRING && isDirectStream === false) {
            ...
            rememberLink(publication.sourceUrl, publication.title);
            const payload = buildPlaybackPayload(publication);
            const delivery = await pushToTelevision(payload);
            store.persist();
            sendJson(response, HTTP_OK, Object.assign({ ok: true }, payload, delivery, {
    ...
    async function handleSendResolvedOffer(response, body) {
        ...
        await publishAndPush(response, resolved,
            String(body.subtitleId || SUBTITLE_OFF), String(body.renditionId || EMPTY_STRING));
    }

    async function handleSendByUrl(response, pageUrl, body) {
        let resolved = null;
        try {
            resolved = await resolveMedia(pageUrl);
            resolved.sourceUrl = pageUrl;
            attachGeneratedSubtitle(resolved);
        } catch (error) {
            sendJson(response, HTTP_SERVER_ERROR, { ok: false, error: error.message });
        ...
        resolvedOffers.set(
            publications.buildPublicationId(pageUrl, EMPTY_STRING, EMPTY_STRING), resolved);
        const offered = resolved.subtitleTracks || [];
        const requested = body.subtitleId;
        const subtitleId = requested === undefined
            ? (offered.length > 0 ? offered[0].identifier : SUBTITLE_OFF)
            : String(requested);
        await publishAndPush(response, resolved, subtitleId, String(body.renditionId || EMPTY_STRING));
    }
    ...
    async function handleRequest(request, response, url) {
        ...
        if (pathName === '/api/links' && request.method === 'POST') {
            ...
            if (/^https?:\/\//i.test(pageUrl) === false) {
                ...
                rememberLink(pageUrl, String(body.value.title || pageUrl));
                store.persist();
        ...
        if (pathName === '/api/links') {
            sendJson(response, HTTP_OK, { items: listLinks() });
            return true;
        }
