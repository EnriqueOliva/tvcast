...
function createTelevisionApi(options) {
    ...
    function buildPlaybackPayload(publication) {
        ...
        const useMaster = hls.hasSeparateAudio(publication);
        ...
        return {
            ...
            mediaUrl: useMaster
                ? `${base}/hls/${publication.id}/master.m3u8`
                : `${base}/hls/${publication.id}/video.m3u8`,
    ...
    async function verifySubtitleSync(publication) {
        const selected = (publication.subtitles || [])
            .find((track) => track.id === publication.selectedSubtitleId);
        if (selected === undefined || selected.cues === null || publication.tracks.video === null) {
            return;
        }
        try {
    ...
    async function handleSend(request, response) {
        ...
        const offerId = String(body.value.offerId || EMPTY_STRING);
        const renditionId = String(body.value.renditionId || EMPTY_STRING);
        const subtitleId = String(body.value.subtitleId || SUBTITLE_OFF);
        ...
        try {
            ...
            if (publication.selectedSubtitleId !== SUBTITLE_OFF && hasCachedOffset === false) {
    ...
    async function handleRequest(request, response, url) {
        ...
        if (pathName.startsWith('/api/catalogue/') && request.method === 'DELETE') {
            ...
            sendJson(response, HTTP_OK, { ok: store.remove(publicationId) });
            return true;
        }
