...
const DURATION_TOLERANCE_RATIO = 0.35;
...
async function loadProviderPayload(provider, metadata, parsed) {
    const seed = await fetchSeed(parsed.tmdbId);
...
async function loadEveryProviderPayload(metadata, parsed) {
    const attempts = PROVIDERS.map((provider) => loadProviderPayload(provider, metadata, parsed)
...
async function resolveTimeline(playlistUrl, maximumHeight) {
    ...
    const variant = pickVariantForCeiling(parsedFirst.variants, ceiling);
...
async function inspectEveryRendition(renditions, expectedSeconds, maximumHeight) {
    return Promise.all(renditions.map((rendition) => inspectRendition(rendition, expectedSeconds, maximumHeight)));
}

async function selectRendition(renditions, maximumHeight, expectedSeconds) {
    const inspections = await inspectEveryRendition(renditions, expectedSeconds, maximumHeight);
    ...
    const usableRenditions = applyDisplayLabels(usable.map((inspection) => inspection.rendition));
    ...
    const findInspection = (rendition) => usable.find((inspection) => inspection.rendition.identifier === rendition.identifier);
...
async function resolveCineby(pageUrl) {
