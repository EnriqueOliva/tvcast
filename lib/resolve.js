...
const ADAPTIVE_RENDITION_ID = 'auto';
...
function buildHeightRenditions(heights) {
    const renditions = heights.map((height) => ({
        identifier: String(height),
        displayLabel: `${height}p`,
        height,
        provider: 'fixed'
    }));
    renditions.push({
        identifier: ADAPTIVE_RENDITION_ID,
        displayLabel: 'Auto',
        height: 0,
        provider: 'adaptive'
    });
...
async function resolveMedia(pageUrl) {
    ...
    if (manifestUrl !== EMPTY_STRING) {
        ...
        const tallest = heights.length > 0 ? heights[0] : 0;
        return buildDirectStream({
            title,
            durationSeconds,
            provider,
            quality: tallest > 0 ? `${tallest}p` : EMPTY_STRING,
            height: tallest,
            streamUrl: manifestUrl,
            streamKind: STREAM_KIND_HLS,
            httpHeaders: info.http_headers || {},
            subtitleTracks,
            renditions: buildHeightRenditions(heights),
            selectedRenditionId: tallest > 0 ? String(tallest) : ADAPTIVE_RENDITION_ID
        });
    }
